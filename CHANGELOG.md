...
- Faculty writes in the administration console are scoped to the caller's own departments. `faculty:create` and `faculty:update` are both HOD defaults, and neither `createFacultyAction` nor `deactivateFacultyAction` narrowed on a department — so an HOD could add staff to a department that was not theirs, and deactivate any faculty member in the college, another department's HOD included. The department workspace had checked all along; the console had not, and `inDeptScope` now lives in `lib/allocation.ts` beside the other scope rules so there is one statement of it rather than two. ([#106](https://github.com/voss-labs/verp/pull/106))
- Removing a student from a lab batch checks the class roster, which adding one already did — the kind of asymmetry that survives review because the reader checks the interesting direction. Bulk student deactivation now refuses a request containing an out-of-scope id instead of quietly deactivating the rest, matching the rule the marks and attendance writes adopted: a partial write reports success for an action that did something other than what was asked. ([#106](https://github.com/voss-labs/verp/pull/106))
