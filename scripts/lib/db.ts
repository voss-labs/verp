...
// Tables that only a verp schema has. The `_migrations` ledger alone is not a
// reliable marker: a database set up with `db:push` (the documented manual path)
// never gets one, so keying off it classified a perfectly healthy verp install
// as "foreign" and made --ci refuse to run. Recognise the schema itself.
const VERP_MARKER_TABLES = ["faculty", "students", "departments", "classes"]
...
export async function inspectDatabase(url: string): Promise<DbState> {
  try {
    ...
    if (
      names.includes("_migrations") ||
      VERP_MARKER_TABLES.every((t) => names.includes(t))
    ) {
